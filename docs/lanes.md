...
| ops-version-control | main checkout | new `ops/` directory (import of `/root/ops/scripts/*` from the VPS, previously unversioned); `scripts/deploy_ops_to_vps.sh` (new); `scripts/monitoring/ops_drift_check.sh` (new) + `scripts/monitoring/daily_checks.sh` (wire-in); `TOOLING_RUNBOOK.md` (new) | **CLAIMED 2026-08-31** — orchestrator session (GCI checkout, cross-repo). Brings the VPS alerting layer under version control per Director ruling: shared infrastructure goes in canon, not a site repo. No script behavior changes — import only, plus the new deploy path and drift check. |
